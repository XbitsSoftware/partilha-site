...
export const formSchema = z
   .object({
      ...
      customer: z.object({
         ...
         document: z
            .string()
            .min(14, "CPF/CNPJ obrigatório")
            .refine(
               (val) => useValidation(val, "cpf") || useValidation(val, "cnpj"),
               {
   ...
   .superRefine((data, ctx) => {
      ...
      if (data.charge.billingType === "CreditCard") {
         ...
         if (
            data.payment.holderInfo.document &&
            !useValidation(data.payment.holderInfo.document, "cpf") &&
            !useValidation(data.payment.holderInfo.document, "cnpj")
         ) {
            ctx.addIssue({
               path: ["payment", "holderInfo", "document"],
         ...
         if (
            ...
            !useValidation(data.payment.holderInfo.email, "email")
         ) {
            ctx.addIssue({
